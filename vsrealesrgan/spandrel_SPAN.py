...
class Conv3XC(nn.Module):
    ...
    def update_params(self):
        device = self.eval_conv.weight.device  # Ensure the tensors are on the correct device

        w1 = self.conv[0].weight.data.clone().detach().to(device)
        b1 = self.conv[0].bias.data.clone().detach().to(device)
        w2 = self.conv[1].weight.data.clone().detach().to(device)
        b2 = self.conv[1].bias.data.clone().detach().to(device)
        w3 = self.conv[2].weight.data.clone().detach().to(device)
        b3 = self.conv[2].bias.data.clone().detach().to(device)
        ...
        sk_w = self.sk.weight.data.clone().detach().to(device)
        sk_b = self.sk.bias.data.clone().detach().to(device)  # type: ignore
    ...
    def forward(self, x):
        x = x.to("cuda")
        # device = next(self.parameters()).device  # Ensure input is on the correct device
        # x = x.to(device)
